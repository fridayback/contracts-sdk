...
class TreasuryCheckScript {
    ...
    static async burn(protocolParams, utxosForFee, utxoForCollateral, utxosSpend, scriptRef, checkTokenScriptRef, groupInfoUtxo, adminNftInfo, changeAddress, signFn, ttl, rawMetaData) {
        ...
        const exUnits = CardanoWasm.ExUnits.new(
            CardanoWasm.BigNum.from_str((5690302) + ''),
        ...
        mintBuilder.add_asset(mint_witnes, assetName, CardanoWasm.Int.from_str('-' + (utxosSpend.length - 1)));
        txBuilder.set_mint_builder(mintBuilder);

        {
            const authorityDatum = utils.genDemoDatum42();
            const holderAddr = CardanoWasm.Address.from_bech32(utxosSpend[0].address);//TreasuryCheckScript.address(groupInfo[contractMgr.GroupNFT.StkVh]);
            const tokenId = TreasuryCheckTokenScript.tokenId();
            ...
            mutiAsset.insert(TreasuryCheckTokenScript.script().hash(), asset);

            let mintedValue = CardanoWasm.Value.new_with_assets(CardanoWasm.BigNum.from_str('' + minAdaWithToken), mutiAsset)
            const output = CardanoWasm.TransactionOutput.new(CardanoWasm.Address.from_bech32(utxosSpend[0].address), mintedValue);
        ...
        for (let i = 0; i < utxosSpend.length; i++) {
            ...
            const exUnits = CardanoWasm.ExUnits.new(
                CardanoWasm.BigNum.from_str((7575293) + ''),
                CardanoWasm.BigNum.from_str((2880092692) + '')
            );
...
class TreasuryScript {
    ...
    static caculateRedeemDataHash(redeemData) {
        ...
        const rawData = pkhPay + pkhStk + policy_id + tokenName + amount + adaAmount + nonceHash + nonceIndex + mode + redeemData.uniqueId + txType + ttl + outputCount;
    ...
    static treasuryRedeemProof(proof) {
        ...
        ls.add(CardanoWasm.PlutusData.new_bytes(Buffer.from(to.pkhStk, 'hex')));
    ...
    static async transferFromTreasury(protocolParams, utxosForFee, utxosToSpend, scriptRefUtxo, groupNFTUtxo, funValueMap, toAddr, redeemProof, utxoForCollateral, treasuryCheckUxto, treasuryCheckRef, changeAddress, evaluateTxFn, signFn, rawMetaData, ttl) {

        const signedTx = await TreasuryScript.transferFromTreasuryWithoutEvaluate(protocolParams, utxosForFee, utxosToSpend, scriptRefUtxo, groupNFTUtxo, funValueMap, toAddr, redeemProof, utxoForCollateral, treasuryCheckUxto, treasuryCheckRef, changeAddress, signFn, rawMetaData, ttl);
        // return signedTx;
    ...
    static async transferFromTreasuryWithoutEvaluate(protocolParams, utxosForFee, utxosToSpend, scriptRefUtxo, groupNFTUtxo, funValueMap, toAddr, redeemProof, utxoForCollateral, treasuryCheckUxto, treasuryCheckRef, changeAddress, signFn, rawMetaData, ttl) {
        const txBuilder = utils.initTxBuilder(protocolParams);
        ...
        for (let i = 0; i < utxosToSpend.length; i++) {
            ...
            //         ex_unit_mem = EX_UINT_MEM_LAST_OF_TWO + EX_UINT_MEM_AUGMENTER_BETWEEN_LAST * (utxosToSpend.length - 2);
            //         ex_unit_cpu = EX_UINT_CPU_LAST_OF_TWO + EX_UINT_CPU_AUGMENTER_BETWEEN_LAST * (utxosToSpend.length - 2);
            //     } else {
            //         ex_unit_mem = EX_UINT_MEM_FIRST_OF_TWO + EX_UINT_MEM_AUGMENTER_BETWEEN_FIRST * (utxosToSpend.length - 2) + EX_UINT_MEM_AUGMENTER_BETWEEN_PRE_NEXT * offset;
            //         ex_unit_cpu = EX_UINT_CPU_FIRST_OF_TWO + EX_UINT_CPU_AUGMENTER_BETWEEN_FIRST * (utxosToSpend.length - 2) + EX_UINT_CPU_AUGMENTER_BETWEEN_PRE_NEXT * offset;
            //     }
            // }
            // console.log('mem:', ex_unit_mem, 'cpu:', ex_unit_cpu);
            const exUnits = CardanoWasm.ExUnits.new(
...
class MappingTokenScript {
    ...
    static async mintWithoutEvaluate(protocolParams, utxosForFee, utxoForCollateral, scriptRef, mintCheckScriptRef, groupNFTUtxo, mintCheckUtxo, redeemProof, changeAddress, signFn, ttl, rawMetaData) {
        ...
        const minAdaWithMintToken = utils.getMinAdaOfUtxo(protocolParams, toAddr, { coins: 1000000, assets: { [redeemProof.tokenId]: redeemProof.amount } });
        const mutiAsset = CardanoWasm.MultiAsset.new();
        const asset = CardanoWasm.Assets.new();
...
class MintCheckScript {
    ...
    static caculateRedeemDataHash(redeemData) {
        ...
        const mode = padding(new BigNumber(redeemData.mode).toString(16));
        const ttl = padding(new BigNumber(redeemData.ttl).toString(16));
        const rawData = pkhPay + pkhStk + policy_id + tokenName + amount + nonceHash + nonceIndex + mode + redeemData.uniqueId + ttl;
    ...
    static async burn(protocolParams, utxosForFee, utxoForCollateral, utxosSpend, scriptRef, checkTokenScriptRef, groupInfoUtxo, adminNftInfo, changeAddress, signFn, ttl, rawMetaData) {
        ...
        for (let i = 0; i < utxosSpend.length; i++) {
            ...
            const scriptHash = utils.addressToPkhOrScriptHash(utxo.address);
            const witness = CardanoWasm.PlutusWitness.new_with_ref(CardanoWasm.PlutusScriptSource.new_ref_input_with_lang_ver(
                CardanoWasm.ScriptHash.from_hex(scriptHash), input, CardanoWasm.Language.new_plutus_v2())
                , CardanoWasm.DatumSource.new_ref_input(input)
                , redeemer);
...
class CheckTokenScriptBase {
    ...
    static async mint(protocolParams, utxosForFee, utxoForCollateral, scriptRef, groupInfoUtxo, adminNftInfo, changeAddress, mintAmount, mintTo, signFn, ttl, rawMetaData) {
